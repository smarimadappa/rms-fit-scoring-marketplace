...
| Customer industry — secondary corroboration only (survey_responses) | `survey_responses.industry_name`, `survey_responses.count` | filter `{"survey_responses.product_id": "<id>"}`, sort `count desc`, ignore null row. **Primary source is public web research (see `scoring-model.md`)** — this reviewer-derived pull is a secondary signal to corroborate or fill gaps, not the default top-3-vertical source. Same explore/model (`categories_products` / `global_lifecycle`) |
